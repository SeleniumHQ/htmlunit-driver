...
public class CorrectEventFiringTest extends JUnit4TestBase {
  ...
  public void testShouldReportTheXAndYCoordinatesWhenClicking() {
    assumeFalse("Skipping test which fails in IE on Sauce", TestUtilities.isInternetExplorer(driver));
  ...
  private static void assertEventFired(String eventName, WebDriver driver) {
    ...
    String text = new WebDriverWait(driver, Duration.ofSeconds(10)).until(elementTextToContain(result, eventName));
